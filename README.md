...
**Point cloud (`las_mgrs_cli`)** — same input/output idea as `docker/pointcloud_converter.sh` with LAS/LAZ/PCD modes.
...
```bash
# Mode 1 (LAS/LAZ): UTM source -> local MGRS output
python3 -m lanelet2_generator.las_mgrs_cli data/merged_clean.las data --utm-frame 32N --voxel-size 0.1 --color-by auto

# Mode 2 (PCD): not georeferenced — `--local-frame` only (UTM/MGRS flags ignored)
python3 -m lanelet2_generator.las_mgrs_cli --input data/input_cloud.pcd --output data/pointcloud_map.pcd \
  --local-frame --voxel-size 0.1

# Mode 3 (PCD): UTM coordinates on disk — no `--local-frame`
python3 -m lanelet2_generator.las_mgrs_cli --input data/input_cloud.pcd --output data/pointcloud_map.pcd \
  --utm-frame 33N --mgrs-grid 33TWN --subtract-xy-from-mgrs --voxel-size 0.1

# Same with flags and an explicit .pcd path (LAS/LAZ):
...
│   ├── las_mgrs_cli.py               #   CLI: LAS/LAZ/PCD -> local PCD (+ optional MGRS-origin offset)
...
- **Point cloud tool:** LAS/LAZ/PCD -> local PCD + `map_projector_info.yaml`
...
Run point cloud conversion:

```bash
# LAS/LAZ mode (UTM source -> local MGRS output)
...
```bash
# PCD not georeferenced — keep XYZ, only downsample
docker/pointcloud_converter.sh \
  --input data/input_cloud.pcd \
  --output data/pointcloud_map.pcd \
  --local-frame --voxel-size 0.1
```

```bash
# PCD with UTM coordinates on disk (omit --local-frame): CRS + optional tile XY snap
docker/pointcloud_converter.sh \
  --input data/input_cloud.pcd \
  --output data/pointcloud_map.pcd \
  --utm-frame 32N --mgrs-grid 32UQU --subtract-xy-from-mgrs --voxel-size 0.1
```
...
### Point cloud conversion (LAS/LAZ/PCD)
...
# PCD UTM on disk (omit --local-frame): CRS + optional tile XY snap
python3 -m lanelet2_generator.las_mgrs_cli --input data/input_cloud.pcd --output data/pointcloud_map.pcd \
  --utm-frame 32N --mgrs-grid 32UQU --subtract-xy-from-mgrs --voxel-size 0.1

# Explicit .pcd path (LAS/LAZ)
python3 -m lanelet2_generator.las_mgrs_cli --input data/merged_clean.las --output data/pointcloud_map.pcd \
  --utm-frame 32N --mgrs-grid 32UQU --voxel-size 0.1 --color-by auto
```
